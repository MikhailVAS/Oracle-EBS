CREATE OR REPLACE VIEW XXTG_NMA_RETIREMENT_V
as
(SELECT
...
SELECT                                                /*--- Расположение ---*/
       ...
       NVL (bk.life_in_months, 0)
           life_in_months,
--            extract(year from rt.date_retired) - extract(year from bk.prorate_date) year,
--            extract(month from rt.date_retired) - extract(month from bk.prorate_date) month,
--            extract(day from rt.date_retired) - extract(day from bk.prorate_date) day,    
       case when extract(year from rt.date_retired) - extract(year from bk.prorate_date) != 0
           then
               case when extract(year from rt.date_retired) - extract(year from bk.prorate_date) in (1,21,31)
                then extract(year from rt.date_retired) - extract(year from bk.prorate_date) ||' год'
                when extract(year from rt.date_retired) - extract(year from bk.prorate_date) in (2,3,4,22,23,24)
                then extract(year from rt.date_retired) - extract(year from bk.prorate_date) ||' года' 
                else extract(year from rt.date_retired) - extract(year from bk.prorate_date) ||' лет'
               end
        end  life_in_years,    
         case when mod(NVL ((extract(month from rt.date_retired) - extract(month from bk.prorate_date)), 0),12) != 0 
                then
                case when mod(NVL ((extract(month from rt.date_retired) - extract(month from bk.prorate_date)), 0),12) = 1 
                      then mod(NVL ((extract(month from rt.date_retired) - extract(month from bk.prorate_date)), 0),12)|| ' месяц'
                     when mod(NVL ((extract(month from rt.date_retired) - extract(month from bk.prorate_date)), 0),12) in (2,3,4)
                       then mod(NVL ((extract(month from rt.date_retired) - extract(month from bk.prorate_date)), 0),12)|| ' месяца'
                     else mod(NVL ((extract(month from rt.date_retired) - extract(month from bk.prorate_date)), 0),12)|| ' месяцев' 
                end     
             end  
           rem_life_in_months, 
       ...
       NVL (
          ...
          CASE EXTRACT(MONTH FROM nvl(to_date(substr(fth.attribute2,0,10),'YYYY.MM.DD'),rt.date_retired))
           ...
           com.chairman_name full_name,
           HR_DATA.job_name job_name,
           HR_DATA.Organzation unit,
           cmv.membership_id membership_id,
           fth.transaction_header_id,
           bk.prorate_date,
           rt.date_retired - bk.prorate_date,
          -- NVL((fdd.DEPRN_RESERVE + fdd.DEPRN_ADJUSTMENT_AMOUNT + NVL(select DEPRN_RESERVE from FA_DEPRN_DETAIL where PERIOD_COUNTER = fdd.PERIOD_COUNTER+1 and asset_id=rt.asset_id),0),0) amor_amount
           NVL(fdd.DEPRN_RESERVE  + nvl((select DEPRN_RESERVE + DEPRN_ADJUSTMENT_AMOUNT from FA_DEPRN_DETAIL where PERIOD_COUNTER = fdd.PERIOD_COUNTER+1 and asset_id=rt.asset_id),0),0) amor_amount
...
--       and fdd.period_counter = dp.period_counter
       AND HR_DATA.person_id(+) = com.CHAIRMAN_ID 
      ...
      -- AND fth.TRANSACTION_TYPE_CODE in ('Полное списание') 
       ...
       AND fth.ATTRIBUTE_CATEGORY_CODE=54
       AND bk.transaction_header_id_in = rt.TRANSACTION_HEADER_ID_IN
       and rt.asset_id= ad.asset_id 
       and EXTRACT(MONTH FROM rt.date_retired)-1 = dp.period_num
