SELECT distinct T_ADD.ASSET_ID,
--       T_ADD.CURRENT_COST_IN,
--       T_ADD.TRANSACTION_HEADER_ID_IN, 
--       T_ADD.CURRENT_COST_OUT,
--       T_ADD.TRANSACTION_HEADER_ID_OUT,
     ...
     --  T_ADD.YTD_DEPRN                           "Амортизация за отч.период", -- Поле 15
       ...
       T_ADD.HEAD_OF_EXP "Статья затрат",    
       T_ADD.CURRENT_COST_IN - T_ADD.CURRENT_COST_OUT "Сумма",   
       T_ADD.TYPE                                "Тип"-- Поле 17
  FROM (SELECT BOOK.ASSET_ID,
               FA_FIN_IN.CURRENT_COST CURRENT_COST_IN,
               FA_FIN_IN.TRANSACTION_HEADER_ID_IN,
               FA_FIN_OUT.CURRENT_COST CURRENT_COST_OUT,
               FA_FIN_OUT.TRANSACTION_HEADER_ID_OUT,
               BOOK.BOOK_TYPE_CODE,  
       ...
       HX.DESCRIPTION HEAD_OF_EXP,
       --'Добавление' TYPE-- Поле 17
       'Корректировка' TYPE-- Поле 17
  ...
  LEFT JOIN FA_FINANCIAL_INQUIRY_COST_V FA_FIN_IN ON 
       FA_FIN_IN.TRANSACTION_HEADER_ID_IN = TRN.TRANSACTION_HEADER_ID AND
       FA_FIN_IN.ASSET_ID = TRN.ASSET_ID AND
       FA_FIN_IN.BOOK_TYPE_CODE = TRN.BOOK_TYPE_CODE  
  LEFT JOIN FA_TRANSACTION_HISTORY_BOOKS_V FA_FIN_OUT ON 
       FA_FIN_OUT.TRANSACTION_HEADER_ID_OUT = TRN.TRANSACTION_HEADER_ID AND
       FA_FIN_OUT.ASSET_ID = TRN.ASSET_ID AND
       FA_FIN_OUT.BOOK_TYPE_CODE = TRN.BOOK_TYPE_CODE 
  LEFT JOIN XXTG_FA_HEAD_OF_EXPENDITURE_V HX ON
       TRN.ATTRIBUTE5 = HX.FLEX_VALUE                
 WHERE 1 = 1
   AND SUBSTR(ASSET.ATTRIBUTE_CATEGORY_CODE,1,2) = '01'
   AND TRN.TRANSACTION_TYPE_CODE = 'ADJUSTMENT'
   --AND BOOK.asset_id=1039661
   AND TRN.ATTRIBUTE_CATEGORY_CODE = 10
   AND TRN.TRANSACTION_DATE_ENTERED BETWEEN TO_DATE('01.01.2020','DD.MM.RRRR') AND TO_DATE('30.06.2020','DD.MM.RRRR')
