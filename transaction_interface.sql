...
/* Transaction Interface Serial Numbers*/
DELETE FROM  inv.MTL_SERIAL_NUMBERS_INTERFACE sn
 WHERE  sn.TRANSACTION_INTERFACE_ID IN
           (SELECT SERIAL_TRANSACTION_TEMP_ID
             FROM inv.MTL_TRANSACTION_LOTS_INTERFACE mli
            WHERE mli.TRANSACTION_INTERFACE_ID IN
                      (SELECT TRANSACTION_INTERFACE_ID
                         FROM inv.MTL_TRANSACTIONS_INTERFACE mti
                        WHERE SOURCE_HEADER_ID = '22402'))
 
/* Transaction Interface Lot*/
DELETE
  FROM inv.MTL_TRANSACTION_LOTS_INTERFACE mli
 WHERE mli.TRANSACTION_INTERFACE_ID IN
           (SELECT TRANSACTION_INTERFACE_ID
...
DELETE
  FROM inv.MTL_TRANSACTIONS_INTERFACE mti
 WHERE --  mti.TRANSACTION_INTERFACE_ID IN (52932119, 52932121)
        SOURCE_HEADER_ID = '22402'
